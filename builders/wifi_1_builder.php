...
foreach(glob("../sites/*/templates/*_branded.html") as $filename){
  ...
  $promo = $image;
  $image = str_replace('http://img2.email2inbox.co.uk/editor/fullwidth.jpg', getURL($brand, 'wifi_1.png'), $image);
  ...
  if(strpos($filename, 'proper_pubs_community') !== false){
    ...
    $voucher = str_replace($voucherSearch, 'YOUR FREE DESSERT VOUCHER', $voucher);
  } else if((strpos($filename, 'proper_pubs_sports') !== false) || (strpos($filename, 'proper_pubs_BNO') !== false)){
    $voucherSearch = 'YOUR VOUCHER';
